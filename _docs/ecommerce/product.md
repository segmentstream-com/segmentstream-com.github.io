...
>При наличии функции быстрого просмотра товара на странице категории рекомендуем следующий подход. При появлении окна быстрого просмотра товара сделайте push в массив changes: digitalData.changes.push(['product',{/*объект product*/}]). Далее необходимо вызвать событие "Viewed Page": digitalData.events.push({name: 'Viewed Page'}). Такой подход произведет полную эмуляцию загрузки страницы с детальным описанием товаров для систем аналитики, ретаргетинга и других маркетинговых систем.
